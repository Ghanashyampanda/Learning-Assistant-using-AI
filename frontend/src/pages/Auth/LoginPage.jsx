...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.error || err.message || 'Failed to login. Please check your credentials.');
      toast.error(err.error || err.message || 'Failed to login.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-50">
      ...
      <div className="relative w-full max-w-md px-6 z-10">
        ...
        {/* Subtle Footer text */}
        <p className="text-center text-xs text-slate-400 mt-8 max-w-[300px] mx-auto leading-relaxed">
