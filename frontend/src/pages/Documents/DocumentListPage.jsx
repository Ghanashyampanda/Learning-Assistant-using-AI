...
import { Plus, Upload, Trash2, FileText, AlertCircle, X } from 'lucide-react';
...
const DocumentListPage = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await documentService.deleteDocument(selectedDoc._id || selectedDoc.id); // Handle both id and _id
      toast.success(`'${selectedDoc.title}' deleted successfully.`);
      setIsDeleteModalOpen(false);
      setSelectedDoc(null);
      setDocuments(documents.filter((d) => (d._id || d.id) !== (selectedDoc._id || selectedDoc.id)));
    } catch (error) {
      ...
      return (
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center max-w-md">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-linear-to-br from-slate-100 to-slate-200 shadow-lg shadow-slate-100/50 mb-6">
              <FileText
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 py-3 px-6 bg-linear-to-r from-emerald-500 to-teal-500 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 active:scale-[0.98]"
            >
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)]">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] bg-size-[24px_24px] opacity-30 pointer-events-none" />
          ...
          <div className="flex gap-3">
            <button
              ...
              >
                {deleting ? (
                  <span className="flex items-center justify-center gap-2">
                    <div className="w-4 h-4 border-whate/30 border-2 border-t-white rounded-full animate-spin" />
                    Deleting...
                  </span>
                ) : (
                  "Delete"
